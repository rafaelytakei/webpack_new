...
export const initPage = (pageName = "") => {
  buildNavBar();
  if(pageName == "") {
    pageName = location.href.split("/").slice(-1)[0];
  }
  if(pageName == "") {
    pageName = 'index.html';
  }
  ...
  barba.init({
    views: [
      ...
      },
      {
        ...
        beforeEnter() {
          buildPage('validations');
        }
      },
      {
        namespace: 'animations',
        beforeEnter() {
          buildPage('animations');
        }
      }
    ],
    transitions:[
      {
        ...
        leave: ({ current }) => {
          ...
          $(current.container).css('position', 'absolute');
          ...
          return animation.finished.then(() => {
            /* $(current.container).css('position', 'static'); */
          });
        },
        enter: ({ next }) => {
...
const buildPage = (pageName) => {
  ...
  } else if(pageName == 'animations') {
    console.log('teste');
    $('#animations-content').append('<section id="content-section" class="container-fluid"></section>');
    $('#animations-content').attr('id', 'animation-page');
    $('#content-section').append('<div id="animation-div"></div>');

    $('#animation-div').append('<button id="start-animation" class="btn btn-info py-3 px-5">Click Me!</button>');
    $('#start-animation').on('click', () => {
       
        const flipAnimation = anime.timeline().add({
